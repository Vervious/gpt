...
>
> Experiments are generally run at a tiny scale (`n_embd = 768`, `n_layer = 12`, `T = 1024`, varying batch size, 30M-100M parameters) on a single A100, H100, or GH200. Even at this scale, transformers are magic.


> [!TIP] 
> Here are some observations that I thought are interesting:
> - GPT is magic and feels almost perfect.
> - The output of `attn` may not need to be put into the residual. As long as it (and `x`) is fed as input into the `mlp`, the network still performs as well (at least at this scale), albeit slightly slower to train. (This is surprising, and I second guess it: intuitively, I would think `mlp` should support a copy operation, and putting `attn` in the residual would be one manifestation of that.)
...
*Question*: For good measure, what if run attn on mlp only?
This is much worse than no-mlp, and also much worse than `self.attn(mlp,x)`. Of all of these experiments (without including the mlp component elsewhere), `self.attn(mlp, x)` is for some reason the best (and `self.attn(mlp, mlp)` is the worst).
```
...
*Hypothesis*:
Now, one last time, why is the residual so important? If I omit the residual entirely, it should be bad.
...
*Out of curiosity*:
It doesn't work particularly well, but what happens if I turn "All Layer Loss" back on, still leaving out the residual?
...
> [!NOTE]
> *Retrospective Note 2/25*. Retrospective notes have not been added yet for the remainder of this journal.
